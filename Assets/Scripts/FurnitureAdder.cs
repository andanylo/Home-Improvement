...
public class FurnitureAdder : MonoBehaviour
{
    ...
    public void recieveFurnituresFromDatabase(string jsonFurniture)
    {
        Dictionary<string, JObject> values =
            JsonConvert
                .DeserializeObject<Dictionary<string, JObject>>(jsonFurniture);
        ...
        foreach (KeyValuePair<string, JObject> entry in values)
        {
            ...
            foreach (KeyValuePair<string, JObject> furniture in obj)
            {
                ...
                Dictionary<string, JValue> parameters =
                    furniture.Value.ToObject<Dictionary<string, JValue>>();

                FurnitureData furnitureData =
                    getFurnitureData(name, furnitureID, parameters);
    ...
    public void addFurniture(FurnitureData data)
    {
        ...
        GameObject prefab =
            Resources.Load<GameObject>("Prefabs/Furnitures/" + data.name);
        ...
        furniture.GetComponent<FurnitureScript>().manager = uimanager;
    ...
    public void updateFurniture(FurnitureData newData)
    {
        foreach (GameObject furniture in uimanager.furnitures)
        {
            ...
            FurnitureScript furnitureScript =
                furniture.GetComponent<FurnitureScript>();
            if (furnitureScript != null)
            {
                //Check ids of furniture data
                if (
                    furnitureScript.furnitureData.furnitureID ==
                    newData.furnitureID
                )
                {
